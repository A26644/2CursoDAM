...
CREATE TYPE ORGANIZADOR_TIPOS AS ENUM ('Universidad', 'Gobierno', 'Centro de Investigacion', 'Empresa', 'Organizacion no gubernamental', 'Otro');
CREATE TYPE PROYECTO_TIPOS AS ENUM ('Cientifica', 'Clinica', 'Social', 'Educativa', 'Mercado', 'Otro');
...
CREATE TYPE HABILIDAD_NIVELES AS ENUM ('Aficionado', 'Principiante', 'Básico', 'Intermedio', 'Avanzado', 'Profesional');
...
CREATE TABLE PROYECTOPERFIL(
	...
	CONSTRAINT UQ_PROYECTOPERFIL UNIQUE(PROYECTOPERFIL_PROYECTO_ID, PROYECTOPERFIL_PERFIL_ID),
...
CREATE TABLE PERFILETIQUETA(
	...
	CONSTRAINT UQ_PERFILETIQUETA UNIQUE(PERFILETIQUETA_PERFIL_ID, PERFILETIQUETA_ETIQUETA_ID)
);
...
ALTER TABLE COLABORADOR ADD CONSTRAINT FK_COLABORADOR 
FOREIGN KEY (COLABORADOR_CURRICULUM_ID)
REFERENCES CURRICULUM(CURRICULUM_ID);
ALTER TABLE CURRICULUM ADD CONSTRAINT FK_CURRICULUM
FOREIGN KEY (CURRICULUM_COLABORADOR_ID)
